...
def distillation_loss(student_logits, teacher_logits, targets, beta=0.5, gate_threshold=1.0):
    ...
    if gate_threshold < 1.0:
        ...
        mask = mask.float()
        distil = F.mse_loss(student_probs * mask, teacher_probs * mask, reduction='sum') / (mask.sum() + 1e-8)
    else:
        distil = F.mse_loss(student_probs, teacher_probs)

    return task_loss + beta * distil

def main():
    if len(sys.argv) != 2:
        print("Usage: python train_distill_flex.py <config_path>")
        sys.exit(1)
    ...
    print(f"Beta: {beta}, Gate threshold: {gate_threshold}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y in train_loader:
            ...
            loss = distillation_loss(student_logits, teacher_logits, y, beta=beta, gate_threshold=gate_threshold)
            loss.backward()
            optimizer.step()
        ...
        metrics.append({"epoch": epoch, "train_loss": train_loss, "train_iou": train_iou, "val_loss": val_loss, "val_iou": val_iou})
        print(f"Epoch {epoch:02d}/{epochs} | train_loss={train_loss:.4f} | train_iou={train_iou:.4f} | val_loss={val_loss:.4f} | val_iou={val_iou:.4f}")

        if val_iou > best_iou:
            best_iou = val_iou
            ckpt = {"epoch": epoch, "model_state": student.state_dict(), "val_iou": val_iou}
            torch.save(ckpt, os.path.join(out_dir, "student_distilled_best.pt"))
            torch.save(ckpt, os.path.join(out_dir, f"student_distilled_best_epoch{epoch}_iou{val_iou:.4f}.pt"))

    pd.DataFrame(metrics).to_csv(os.path.join(out_dir, "metrics.csv"), index=False)
    print(f"Saved metrics CSV -> {os.path.join(out_dir, 'metrics.csv')}")
    print(f"Best val IoU: {best_iou:.6f}")
